...
// 1. อ่านและสกัด (Extract) เฉพาะ Mermaid Code จากไฟล์ config- และ diagram-
function loadDiagramsFromSpecFiles(dirPath) {
	if (!fs.existsSync(dirPath)) return [];

	const files = fs.readdirSync(dirPath);
	const targetFiles = files.filter(f => {
		const isTargetName = f.startsWith('config-') || f.startsWith('diagram-');
		const isSpecFile = f.endsWith('.spec.js') || f.endsWith('.js');
		if (skipZenumlDiagrams && f.startsWith('diagram-zenuml.')) {
			return false;
		}
...
test.describe('Mermaid Options Dynamic Combination Tests', () => {
	...
	test('Render Mermaid combinations across all config-* and diagram-* spec files', async ({ page }) => {
		if (skipZenumlDiagrams) {
			console.log('⚠️ SKIP_ZENUML_DIAGRAMS=1 -> ข้ามไฟล์ diagram-zenuml.spec.js');
		}

		console.log(`\n🔍 ดึง Mermaid Code จากไฟล์ config-*/diagram-*: รวม ${extractedDiagrams.length} บล็อก`);
