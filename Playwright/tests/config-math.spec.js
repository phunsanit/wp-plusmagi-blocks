...
const { mathConfigCases } = options.configMath;
...
test.describe('Mermaid config - Math', () => {
  ...
  for (const optionCase of mathConfigCases) {
    test(`accepts math config option: ${optionCase.name}`, async ({ page }) => {
...
flowchart LR
  A["$$x^2$$"] --> B["$$\\sqrt{x+3}$$"]
  B --> C[Math Config Option]`, 'Math Config Option');
    });
  }
});